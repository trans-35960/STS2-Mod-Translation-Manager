...
![모드 관리 버튼 안내](assets/screenshots/mods-numbered-actions.png)

1. **보기 전환**: 모드 목록을 상세 보기와 심플 보기 사이에서 전환합니다.
2. **바닐라 실행**: 모드를 비활성화한 안전한 상태로 게임을 실행하는 흐름을 시작합니다.
3. **현재 모드 실행**: 현재 활성화된 모드 구성을 기준으로 게임을 실행합니다.
4. **활성화 토글**: 해당 모드를 활성화하거나 비활성화합니다.
5. **번역 도구 열기**: 해당 모드의 언어 리소스를 번역 워크벤치에서 엽니다.
6. **경로 열기**: 해당 모드 파일 또는 폴더 위치를 엽니다.
7. **언어 리소스 추출**: 모드 패키지에서 번역 가능한 언어 파일을 추출합니다.
8. **삭제**: 해당 모드를 삭제하거나 보관 대상에서 제거합니다.
...
![번역 도구 버튼 안내](assets/screenshots/translation-numbered-actions.png)

1. **저장**: 현재 번역 시트의 편집 내용을 저장합니다.
2. **검증**: 변수, 태그, 줄바꿈 등 구조가 원본과 어긋난 항목을 검사합니다.
3. **번역 저장/적용**: 번역 시트를 저장하고 대상 언어 파일에 반영합니다.
4. **실험적 번역 모드 내보내기**: 번역 결과를 별도 패치 모드 형태로 내보냅니다.
5. **닫기**: 현재 번역 작업 세션을 닫고 시작 화면으로 돌아갑니다.
6. **CSV/JSON 매칭**: 외부에서 작업한 CSV 또는 JSON 번역값을 현재 시트에 매칭해 가져옵니다.
7. **CSV 내보내기**: 현재 번역 시트를 CSV로 내보내 외부 편집기에 사용할 수 있게 합니다.
8. **번역용 JSON 내보내기**: AI 번역이나 검토에 사용할 간소화 JSON을 내보냅니다.
